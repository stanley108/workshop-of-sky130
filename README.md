...
<img src=https://gitlab.com/gab13c/openlane-workshop/-/blob/master/images/10.png>
...
Each design hierarchy comes with three distinct components:
...
Ensure the WNS is an acceptable number, if not please adjust the clock period to fix STA errors.
Day 2 - Chip Floorplanning
In Floorplanning we set the:
1) 	Die Area
2) 	Core Area
3) 	Core Utilization
4) 	Aspect Ratio
...
Two key descriptions of a floorplan are utilization and aspect ratio. The amount of area of the die core the standard cells are taking up is called utilization. This is given as a percentage normally. We normally specify utilization as a utilization:

Normally we go for 50-70% utilization to, or utilization factor of 0.5-0.7. Allows for optimization of placement and realizable routing of a system. Aspect ratio can specify the shape of your chip by:
...
Power Planning
Power planning during the Floorplanning phase is essential to lower noise in digital circuits attributed to voltage droop and ground bounce. Coupling capacitance is formed between interconnect wires and the substrate which needs to be charged or discharged to represent either logic 1 or logic 0. When a transition occurs on a net, charge associated with coupling capacitors may be dumped to ground. If there are not enough ground taps charge will accumulate at the tap and the ground line will act like a large resistor, raising the ground voltage and lowering our noise margin. An illustration is given below:

To bypass this problem a robust PDN with many power strap taps are needed to lower the resistance associated with the PDN:
...
To view placement in Magic mirrors viewing floorplanning:
...
Day 3 
OpenLANE has the benefit of allowing changes to internal switches of the ASIC design flow on the fly. This allows users to experiment with floorplanning and placement without having to reinvoke the tool. Day 3 will investigate 
Spice Simulations
...
Run: plot <node> vs <node> to obtain output waveform
...
Spice Wrapper Creations for Simulation
